# Generated by Django 2.1.4 on 2019-08-15 15:50

from django.db import migrations, models
import uuid


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='menccheck',
            fields=[
                ('FID', models.UUIDField(default=uuid.uuid1, primary_key=True, serialize=False)),
                ('FMecserialFID', models.CharField(blank=True, max_length=32, null=True)),
                ('FCheckPersonID', models.CharField(blank=True, max_length=32, null=True, verbose_name='检查人员ID')),
                ('FCheckdate', models.DateField(blank=True, null=True, verbose_name='检查日期')),
                ('FCheckitemID', models.CharField(blank=True, max_length=32, null=True, verbose_name='检查项目')),
                ('FCheckresult', models.IntegerField(blank=True, choices=[(None, '请选择数据'), (0, '正常'), (1, '异常')], null=True, verbose_name='检查结果')),
                ('FDesc', models.CharField(blank=True, max_length=1024, null=True, verbose_name='备注')),
                ('CREATED_PRJ', models.CharField(blank=True, max_length=32, null=True, verbose_name='所属项目')),
                ('CREATED_ORG', models.CharField(blank=True, max_length=32, null=True, verbose_name='创建组织')),
                ('CREATED_BY', models.CharField(blank=True, max_length=32, null=True, verbose_name='创建人')),
                ('CREATED_TIME', models.DateTimeField(blank=True, null=True, verbose_name='创建时间')),
                ('UPDATED_BY', models.CharField(blank=True, max_length=32, null=True, verbose_name='更新人')),
                ('UPDATED_TIME', models.DateTimeField(auto_now=True, verbose_name='更新时间')),
            ],
            options={
                'db_table': 'T_MecCheckLog',
            },
        ),
    ]
